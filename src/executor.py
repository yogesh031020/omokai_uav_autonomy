...
class DroneExecutor:
    ...
    # ------------------------------------------------------------------
    # Physics warmup: The Gazebo GUI can pause the physics engine when
    # it connects. We fix this by programmatically sending an "unpause"
    # command to the Gazebo world via the gz service API, then verifying
    # that telemetry data is actually flowing before proceeding.
    # ------------------------------------------------------------------
    async def wait_for_physics_ready(self):
        if self.dry_run:
            return
        ...
        # Step 1: Force-unpause Gazebo using the gz service command.
        # This sends a WorldControl message with pause=false to the
        # running Gazebo server, regardless of the GUI's pause state.
        gz_bin = shutil.which("gz")
        if gz_bin:
            print("Sending unpause command to Gazebo world ...")
            for world_name in ["iris_runway", "default"]:
                try:
                    subprocess.run(
                        [
                            gz_bin, "service",
                    ...
                    pass
            # Give the physics engine a moment to resume
            await asyncio.sleep(2)
        else:
            print("gz CLI not found -- skipping unpause (non-Gazebo mode).")

        # Step 2: Verify telemetry is flowing (simulation clock is ticking)
        print("Verifying simulator physics is running ...")
        update_count = 0
        async for pos in self.drone.telemetry.position():
            update_count += 1
            if update_count >= 3:
                print("Simulator physics active -- clock is ticking.")
                break
    ...
    async def wait_for_ready(self):
        ...
        # First ensure the simulation clock is actually running
        await self.wait_for_physics_ready()

        import subprocess
        import shutil

        async def spam_unpause():
            gz_bin = shutil.which("gz")
            if not gz_bin:
                return
            while True:
        ...
        unpause_task = asyncio.create_task(spam_unpause())
        ...
        try:
            ...
            # We use an explicit timeout because MAVSDK sometimes misses the home position broadcast
            try:
                async def get_home():
                    async for pos in self.drone.telemetry.home():
                        return pos
                
                pos = await asyncio.wait_for(get_home(), timeout=5.0)
                ...
                print("Warning: Timeout waiting for home position stream.")
                # Fallback to current position which is virtually identical on the ground
                async for pos in self.drone.telemetry.position():
                    self.home_lat = pos.latitude_deg
                    self.home_lon = pos.longitude_deg
                    print(f"Fallback home position (current): lat={self.home_lat:.6f}  lon={self.home_lon:.6f}")
                    break
        finally:
            unpause_task.cancel()
